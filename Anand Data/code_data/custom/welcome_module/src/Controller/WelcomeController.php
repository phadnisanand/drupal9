...
class WelcomeController extends ControllerBase {
  ...
  public function showcontent(Node $node) {
      $build = [
        '#markup' => 'Hello, from ' . $node->getTitle(),
      ];
      return $build;
  }
   ...
   *   Return markup array.
   */
  public function content() { //die('hi');

    $node_storage = \Drupal::entityTypeManager()->getStorage('node');
    $node = $node_storage->load(1);
    //echo $node->title->value;  exit;
...
//echo '<pre>'; print_r($data[0][0]); exit;
  ...
  public function index($first_name) {
